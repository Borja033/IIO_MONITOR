...
static void shared_memory_event_write(struct iio_event_data *event){
    ...
    strcpy(memory->ev_type,iio_ev_dir_text[dir]);
	strcpy(memory->ch_type,iio_chan_type_name_spec[type]);
    memory->chn = chan;
    memory->tmpstmp = event->timestamp;
...
int main(int argc, char **argv){
	...
	fd = open(chrdev_name, O_WRONLY | O_NONBLOCK);
	if (fd == -1) {
		ret = -errno;
		printf("%d",ret);
	...
	while (true) {
		...
		shared_memory_event_write(&event);
		//print_event(&event);
	}
